            ...
            <DialogPanel className="pointer-events-auto relative w-screen max-w-md transform transition duration-500 ease-in-out data-[closed]:translate-x-full sm:duration-700">
              <TransitionChild>
                <div className="absolute left-0 top-0 -ml-8 flex pr-2 pt-4 duration-500 ease-in-out data-[closed]:opacity-0 sm:-ml-10 sm:pr-4">
                  <button
                    type="button"
                    onClick={() => setOpenProductForm(false)} // Close form on clicking X
                ...
                <Formik
                  ...
                  onSubmit={(values) => {
                    if (showProductButton) {
                      updateSingleProduct(values);
                    } else {
                      setProduct(values);
                    }
                    setOpenProductForm(false); // Close form on successful submission
                  }}
                >
                  {() => (
                    <Form className="px-4 sm:px-6 flex flex-col gap-4">
                      ...
                      <DialogTitle className={"text-gray-400"}>Product Category</DialogTitle>
                      <Field
                          as="select"
