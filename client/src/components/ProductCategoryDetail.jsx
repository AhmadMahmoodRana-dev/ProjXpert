...
const ProductCategoryDetail = () => {
  const { SingleProductCategory, openProductCategoryDetailPage,
    setOpenProductCategoryDetailPage } =
    useContext(Context);
  return (
    <Dialog
      open={openProductCategoryDetailPage}
      onClose={setOpenProductCategoryDetailPage}
            ...
            >
              <TransitionChild>
                <div className="absolute left-0 top-0 -ml-8 flex pr-2 pt-4 duration-500 ease-in-out data-[closed]:opacity-0 sm:-ml-10 sm:pr-4">
                  <button
                    type="button"
                    onClick={() => setOpenProductCategoryDetailPage(!openProductCategoryDetailPage)}
              ...
              <div className="flex h-full flex-col overflow-y-scroll bg-white py-6 shadow-xl px-4">
                ...
                <h1 className="mt-4 font-bold text-4xl italic text-gray-600">{`${SingleProductCategory?.name}`}</h1>
                <hr className="border border-gray-300 mt-6" />
                <div className="flex flex-col justify-around w-full h-[50%] mt-10">
                  ...
                  <div className="flex  items-center gap-4">
                    ...
                    <p className=" text-lg font-bold italic text-gray-600">
                      {SingleProductCategory?.description}
                    </p>
                  </div>
                  <div className="flex  items-center gap-4">
                    <label className="block text-md font-bold italic text-gray-700 min-w-[100px] max-w-[100px]">
                    ...
                    <p className=" text-lg font-bold italic text-gray-600">
                      {SingleProductCategory?.color || "Null"}
                    </p>
                  </div>
                  <div className="flex  items-center gap-4">
                    <label className="block text-md font-bold italic text-gray-700 min-w-[100px] max-w-[100px]">
                    ...
                    <Switch
          checked={SingleProductCategory?.enabled} 
          onCheckedChange={() => {} }
          disabled={SingleProductCategory?.enabled} 
        />
                  </div>
